$(document).on("click", ".add-case", function(){
    row = $(this).data("row");
    let product_name = $("#id_orders-"+row+"-product option:selected").text();
    let product_id = $("#id_orders-"+row+"-product").val();
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.success == true){
                ...
                $(`#id_orders-${row}-case_pack`).append(new Option(text, data.product_case[0]));
                $(`#id_orders-${row}-case_pack option:last`).attr("data-qty", data.product_case[1]);
                $.toast({
